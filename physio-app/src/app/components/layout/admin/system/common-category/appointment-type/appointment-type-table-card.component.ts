  ...
  template: `
    <div class="bg-surface rounded-[6px] border border-gray-200 h-full overflow-hidden">
      ...
      >  
        ...
        <ng-template appColumnDef="defaultDuration" headerLabel="Duration" headerClass="text-right" cellClass="text-right" let-item>
          <div class="text-sm">{{ item.defaultDuration }}</div>
        </ng-template>

        <ng-template appColumnDef="isEmergency" headerLabel="Emergency" headerClass="text-center" cellClass="text-center" let-item>
          <boo-icon name="circle-check" *ngIf="item.IsEmergency"  />
        </ng-template>

        <ng-template appColumnDef="consultationFee" headerLabel="Consultation Fee" headerClass="text-right" cellClass="text-right" let-item>
