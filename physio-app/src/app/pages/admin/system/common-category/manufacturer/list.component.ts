...
@Component({
    ...
    imports: [
        ...
        ButtonIconComponent,
        CommonCategoryManufacturerTableCardComponent,
        CommonCategoryManufacturerDrawerComponent
    ],
    templateUrl: `./list.component.html`,
    host: { class: 'block h-full min-h-0' }
})

export class CommonCategoryManufacturerListComponent implements OnInit {
    @ViewChild('tableHost', { static: false }) tableHost?: ElementRef<HTMLElement>;

    // #region Inputs, Outputs, Properties
    tableData = signal<PaginationData<Manufacturer> | null>(null);
    params = {
        pageNumber: 1,
        pageSize: 5,
        search: '',
    ...
    onSaveSuccess(result: Manufacturer) {
        this.onCloseDrawer();
    ...
    onDelete(id: string | null) {
        ...
        this.dialogSrv.confirmDelete(() => {
            ...
            if (isLastItemOnPage && isNotFirstPage) {
                ...
                this.manufacturerSrv.delete(id).subscribe({
                    next: () => {
            ...
            this.toastSrv.success("Deleted 1 item.")
            this.manufacturerSrv.delete(id)
                .pipe(
                    catchError(_ => {
    ...
    onBulkAction(event: { action: BulkAction; ids: (number | string)[]; selectAllPages: boolean }) {
        ...
        if (action.key === 'delete') {
            const calls = ids.map(id => this.manufacturerSrv.delete(String(id)).pipe(
