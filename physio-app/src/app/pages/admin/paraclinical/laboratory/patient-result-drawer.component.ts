...
@Component({
  selector: 'laboratory-patient-result-drawer',
  standalone: true,
    ...
    <drawer [isOpen]="isOpen" [width]="720" (close)="onClose()">
      <div class="flex flex-col h-full bg-surface relative">
        <div class="flex-none px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-surface z-10 sticky top-0">
          <div>
            <h2 class="text-xl font-bold text-primary leading-none mb-1">Patient Laboratory Results</h2>
        ...
        <div class="flex-1 overflow-y-auto bg-surface" custom-scrollbar>
          ...
          <ng-container *ngIf="!isLoading() && summary()">
            ...
            <!-- Laboratory history -->
            <div class="px-6 py-5">
              <h3 class="text-sm font-semibold text-gray-800 mb-3">Laboratory History</h3>
              <div *ngIf="!history().length"><boo-empty-state icon="flask-conical" title="No laboratory history for this patient"></boo-empty-state></div>
              <div *ngIf="history().length" class="space-y-2">
                <div *ngFor="let o of history()" class="border border-gray-200 rounded-lg p-3">
                  <div class="flex items-center justify-between gap-2">
                    <div class="min-w-0">
                      <div class="text-sm font-medium text-gray-800 truncate">{{ o.orderNumber }} — {{ testNames(o) }}</div>
                      <div class="text-xs text-gray-500">{{ o.orderTime | date:'medium' }} · {{ o.orderingDoctorName }}</div>
                    </div>
                    <boo-status-badge [label]="o.verificationStatus" [tone]="verificationTone(o.verificationStatus)"></boo-status-badge>
                  </div>
                </div>
              </div>
            </div>
...
export class LaboratoryPatientResultDrawerComponent implements OnChanges {
  @Input() isOpen = false;
  @Input() mrn: string | null = null;
  @Output() close = new EventEmitter<void>();

  isLoading = signal(true);
  summary = signal<LabPatientResultSummary | null>(null);
  history = signal<LabOrderRow[]>([]);

  constructor(private srv: LaboratoryService, private toastSrv: ToastService, private router: Router) { }

  ngOnChanges(): void {
    if (!this.isOpen || !this.mrn) return;
    this.isLoading.set(true);
    forkJoin({
      summary: this.srv.getPatientSummary(this.mrn),
      history: this.srv.getPatientHistory(this.mrn),
    }).subscribe({
  ...
  viewEmr(): void {
    this.router.navigate(['/admin/clinic/medical-record'], { queryParams: { patientId: this.mrn } });
  }
