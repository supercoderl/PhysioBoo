...
import { CommonCategorySupplierDrawerComponent } from "../../../../../components/layout/admin/system/common-category/supplier/supplier-drawer.component";
import { CommonCategorySupplierTableCardComponent } from "../../../../../components/layout/admin/system/common-category/supplier/supplier-table-card.component";
...
@Component({
    selector: 'common-category-supplier-list',
    standalone: true,
    imports: [
        SharedModule,
        AdminContentHeaderComponent,
        ButtonIconComponent,
        CommonCategorySupplierTableCardComponent,
        CommonCategorySupplierDrawerComponent
    ],
    templateUrl: `./list.component.html`,
    host: { class: 'block h-full min-h-0' }
})

export class CommonCategorySupplierListComponent implements OnInit {
    @ViewChild('tableHost', { static: false }) tableHost?: ElementRef<HTMLElement>;

    // #region Inputs, Outputs, Properties
    tableData = signal<PaginationData<Supplier> | null>(null);
    params = {
        pageNumber: 1,
        pageSize: 5,
        search: '',
    ...
    onDelete(id: string | null) {
        ...
        this.dialogSrv.confirmDelete(() => {
            ...
            if (isLastItemOnPage && isNotFirstPage) {
                ...
                this.supplierSrv.delete(id).subscribe({
                    next: () => {
    ...
    onBulkAction(event: { action: BulkAction; ids: (number | string)[]; selectAllPages: boolean }) {
        ...
        if (action.key === 'delete') {
            const calls = ids.map(id => this.supplierSrv.delete(String(id)).pipe(
