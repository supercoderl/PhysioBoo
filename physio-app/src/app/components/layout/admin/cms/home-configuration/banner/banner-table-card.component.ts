...
@Component({
  selector: 'home-config-banner-table-card',
  standalone: true,
  imports: [
    SharedModule,
  ...
  template: `
    <div class="bg-surface rounded-[6px] border border-gray-200 h-full overflow-hidden">
      ...
      >
        ...
        <ng-template appColumnDef="imageUrl" headerLabel="Image" headerClass="text-left" width="72px" let-item>
          <img
            *ngIf="item.imageUrl; else noImage"
            [src]="item.imageUrl"
            [alt]="item.title"
            class="w-14 h-9 rounded-md object-cover bg-gray-100"
          />
          <ng-template #noImage>
            <div class="w-14 h-9 rounded-md bg-gray-100 flex items-center justify-center">
        ...
        <ng-template appColumnDef="title" headerLabel="Title" headerClass="text-left" [sortable]="true" let-item>
          <div class="min-w-0" (click)="onEditClick(item.id)">
            ...
            <div class="truncate text-xs text-secondary">{{ item.subtitle }}</div>
          </div>
        </ng-template>
...
export class HomeConfigBannerTableCardComponent {
  // #region Inputs, Outputs, Properties
  @Input() data: PaginationData<Banner> | null = null;
