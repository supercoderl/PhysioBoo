...
export class CommonCategoryHospitalGroupDrawerComponent implements OnChanges {
    ...
    async onSave() {
        ...
        const request$ = this.currentId
            ? this.hospitalGroupSrv.update(formData)
            : this.hospitalGroupSrv.create(formData);

        try {
            await firstValueFrom(request$);
            const response = await firstValueFrom(this.hospitalGroupSrv.search_by_id({ id: targetId }));
