...
namespace PhysioBoo.Application.Test
{
    public class CommandHandlerFixtureBase
    {
        ...
        protected CommandHandlerFixtureBase()
        {
            ...
            User.GetUserRole().Returns(Role.ADMIN.ToString());
        ...
        {
            Bus.Received(1).RaiseEventAsync(Arg.Is<DomainNotification>(not =>
                not.Code == errorCode &&
                not.Value == message
            ));
        ...
        public CommandHandlerFixtureBase VerifyNoRaisedEvent<TEvent>() where TEvent : DomainEvent
        {
